...
LIVE=$(kubectl -n ollama-demo get deploy ollama-demo \
    -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null || echo "")

if [ -z "$LIVE" ]; then
    echo "No ollama-demo deployment found — skipping image restore."
elif [ "${LIVE##*/}" = "${WANT##*/}" ]; then
    echo "Dev already running main ($MAIN_SHA) — no restore needed."
else
    echo "Dev is running ${LIVE##*/}, restoring to $MAIN_SHA"
    kubectl -n ollama-demo set image deployment/ollama-demo "ollama-demo=$WANT"
    if ! kubectl -n ollama-demo rollout status deployment/ollama-demo --timeout=90s; then
        ...
        kubectl -n ollama-demo rollout undo deployment/ollama-demo || true
        kubectl -n ollama-demo rollout status deployment/ollama-demo --timeout=90s || true
    fi
fi
